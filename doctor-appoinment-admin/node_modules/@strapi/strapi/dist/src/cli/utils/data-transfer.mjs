...
const formatDiagnostic = (operation, info)=>{
    ...
    const getLogger = ()=>{
        if (!logger) {
            logger = createLogger(configs.createOutputFileConfiguration(`${operation}_${Date.now()}.log`, {
                level: 'info',
...
export { DEFAULT_IGNORED_CONTENT_TYPES, abortTransfer, buildTransferTable, createStrapiInstance, excludeOption, exitMessageText, formatDiagnostic, getAssetsBackupHandler, getDefaultExportName, getDiffHandler, getTransferTelemetryPayload, loadersFactory, onlyOption, parseRestoreFromOptions, setSignalHandler, shouldSkipStage, throttleOption, validateExcludeOnly };
